...
<html>
    ...
    <script type="text/javascript">
    	...
    	function checkValue(){
    		...
    		var form = document.frm;
  			...
  			if(form.shopStartTime.value==0){
  				alert("시작 시간을 정해주세요.");
  				return false;
  			}
  			if(form.shopEndTime.value==0){
  				alert("마감 시간을 정해주세요");
  				return false;
  			}
  ...
  <body>
  		...
  		<form:form commandName="shopCommand" name="frm" action="shopJoin.com" method="post" enctype="multipart/form-data" onsubmit="return checkValue()" > 
        ...
        <form:select style="width:150px;" id="shopLentType" path="shopLentType">  
        ...
        <div class="col-sm-6">
          <form:input class="form-control" path="shopName" id="shopName" type="text" placeholder="점포 이름을 입력해주세요" />
        </div>
        </div>
         <div class="form-group">
         	<label class="col-sm-3 control-label" for="inputId">점포 업종 유형</label>		
        <div class="col-sm-6">
 			<form:select style="width:230px;" id="shopType" path="shopType" onChange="fillSelectFromArray1(this.form.WORK_DISTRICT_NAME2,((this.selectedIndex == -1) ? null : modelinfo[this.selectedIndex]));">
        ...
        <div class="col-sm-6">
        	<form:input class="form-control" path="shopMonth" id="shopMonth" type="text" placeholder="ex)100000"/>
        </div>
        </div>
         <div class="form-group">
        	<label class="col-sm-3 control-label" for="inputName">점포 보증 금액</label> <!-- Long 타입이므로 null값이 들어가지 못하게 JS처리 필요 -->
        <div class="col-sm-6">
        	<form:input class="form-control" path="shopDeposit" id="shopDeposit" type="text" placeholder="ex)100000"/>
        </div>
        </div> 
         <div class="form-group">
        	<label class="col-sm-3 control-label" for="inputName">점포 대여 금액</label> <!-- Long 타입이므로 null값이 들어가지 못하게 JS처리 필요 -->
        <div class="col-sm-6">
        	<form:input class="form-control" path="shopLentPay" id="shopLentPay" type="text" placeholder="ex)100000"/>
        </div>
        </div>  
		...
		<div class="col-sm-6">
		    <form:textarea path="shopDetail" rows="10" cols="40" placeholder="점포 특이사항을 입력해주세요" id="shopDetail"></form:textarea>
        </div>
		</div>
